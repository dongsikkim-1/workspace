<template>
    <div id="app">
        ...
        <span>JSON 출력</span>
        <div id="result" v-for="item in result" v-bind:key="item.id" class="container">
            <xmp>{{item.id}}</xmp><xmp>{{item.title}}</xmp>
        </div>
        <example-com title="홍길동">
...
import exampleCom from '@/components/ExampleCom.vue'
import ExampleCom from './components/ExampleCom.vue'

export default {
  components: { ExampleCom },
    name : "app",
    data() {
        return {
