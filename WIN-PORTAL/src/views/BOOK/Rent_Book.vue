  ...
  <div>
    ...
    <div class="contents">
      ...
      <div class="common-btnwrap">
        <!-- <b-button size="sm" variant="primary" @click="test">테스트</b-button>&nbsp; -->
        <b-button size="sm" variant="primary" @click="search">조회</b-button>&nbsp;
        <b-button size="sm" variant="primary" @click="rent">대여</b-button>
      </div>
      <!-- //공통 버튼 영역// -->
...
<script>
/*eslint-disable no-unused-vars, no-empty, no-unreachable, no-unused-vars*/ // 변수 선언 후 사용하지 않거나, 무의미한 띄어쓰기가 코드에 들어간 경우 에러를 내뱉는 경우를 방지
...
import { Book_Status, Book_Rent, TEST_RENT_BOOK_START, TEST_RENT_BOOK_STOP } from "@/api/Rent_Book.js"; // 도서분류 콤보 , 도서현황 조회, 도서 대여
...
import CommonUtility from '../../assets/js/CommonUtility.js';
...
export default {
  ...
  methods: {
    onRecieve_Message(event) {
      ...
      console.log('rent_book message', event.data); //—> ‘hello’가 출력된다.  
      ...
      this.selected2 = event.data.data;
    },
     ...mapActions('UserInfo', ['Set_UserInfo']),
    // 초기 시용자 Screen 설정 이벤트
    test(){
      TEST_RENT_BOOK_START();
      //this.$store.commit('UserInfo/Set_UserInfo', {user_id:'ttt', position1:'23'});
   ...
   // 조회조건 도서분류 콤보 바인딩 이벤트
    ...
    async search() {
      ...
      try {
        ...
        // 내 대여 목록 체크 박스가 체크된 경우
        if(this.chkSTATUS === 'Y') {
          USER_ID = getToken("USER_ID") // 로그인 한 사용자 ID를 담는다
        }
    ...
    // 도서 대여 이벤트
    async rent() {
      // var t = this.$store.getters.Get_UserInfo;
      // //alert(this.$store.getters["UserInfo/Get_UserInfo"].user_id)
      // return;
      var msgBox;
      // 로그인 정보가 없는 경우 대여 불가
