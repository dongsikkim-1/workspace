...
import { Book_Management,  Book_SAVE, Book_DELETE} from "@/api/Management_Book.js"; // 도서 분류 콤보 , 도서현황 조회, 도서 위치 콤보, 도서 저장, 도서 삭제
...
export default {
  ...
  methods: {
    ...
    async loccombo(){
      try
      {
        let loc_combo = await getCommonCode('POS');
        
        //조회된 데이터가 null이거나 undefined인 경우
        if(Utility.fn_IsNull(loc_combo[0].CODE_NO))
        {
          return;
        }
        ...
        {
          arr[i] = {
            text: loc_combo[i]["CODE_NAME"], // 코드명
            value: loc_combo[i]["CODE_NO"] // 코드
          };
        }
