  ...
  methods:{
    ...
    async search(){        
      try
      {
        ...
        book_kind = await getCommonCode('BK');
        // 조회된 데이터가 0보다 큰경우
        if(Utility.fn_IsNull(book_kind[0].CODE_NO))
        {
          return; 
        }
        ...
        { 
          ...
          { 
            ...
            if(gubun_arr[arr_i] === book_kind[idx]["GUBUN"])
            {
              // 임시 배열 변수에 값을 넣는다.
              Temp_ChkBox ={
                text : book_kind[idx]["CODE_NAME"],   // 도서분류 text
                value : book_kind[idx]["CODE_NO"],  // 도서분류 value
    ...
    apply(){
     ...
     for(var i=0;  i < this.selected.length; i++)
     {
       for(var idx = 0; idx < book_kind.length; idx++)
       {
         if(this.selected[i] === book_kind[idx]["CODE_NO"])
         {
            CODE_CD= CODE_CD+book_kind[idx]["CODE_NO"]+","; // 체크된 체크박스 값 도서분류 코드를 String 형식 으로 만들어준다.
            CODE_NM= CODE_NM+book_kind[idx]["CODE_NAME"]+","; // 체크된 체크박스 값 도서분류 코드명을 String 형식 으로 만들어준다.
         }
       }
     }
