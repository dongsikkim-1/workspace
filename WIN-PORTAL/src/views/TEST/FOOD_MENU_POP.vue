<template>
  <div>
    <div class="modal-header">
      <h4 class="modal-title" id="myModalLabel">메뉴 미리보기</h4>
      ...
      <div class="contents">
        <div class="mobile_modal_form">
          <div class="modal_form">
            <div v-for="options2 in FOOD_DATA" v-bind:key="options2.TEXT">
              <b-form-group
                :label="options2.TEXT"
                style="font-size:20px; font-weight:bold; text-decoration:underline;"
              ></b-form-group>
              <img :src="options2.FOOD_IMG" style="width:400px; height:200px; margin-left: 50px"/>
              <!-- <img v-for="(options3, j) in options2" v-bind:key="j" v-bind:src="options3.FOOD_IMG" style="width:400px; height:200px; margin-left: 50px"/> -->
                <!-- 버튼 2개-->
              
          </div>
          <div class="col-md-12 col-sm-12" id = "test1" style="text-align: center;" >
            <b-button @click="close">닫기</b-button>
...
/*eslint-disable no-undef*/ // 변수 선언 후 사용하지 않거나, 무의미한 띄어쓰기가 코드에 들어간 경우 에러를 내뱉는 경우를 방지
// import 영역
...
export default {
  ...
  mounted(){ 
    ...
    for (var i = 0; i < this.TARGET.today_combo_data.length; i++) {
      if (this.TARGET.today_combo_data[i].FOOD_IMG == '') {
        
      } else {
        this.FOOD_DATA.push(this.TARGET.today_combo_data[i])
      }
    }
    console.log('this.FOOD_DATA', this.FOOD_DATA)
  },
  // 이벤트 선언부
  methods:{
