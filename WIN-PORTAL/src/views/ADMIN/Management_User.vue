        ...
        <fieldset>
          <div class="row">
            <div class="col-md-3 col-sm-6">
              ...
              <div class="col-md-3 col-sm-6">                                                                                        
                <div class="col-md-8 col-sm-8 col-xs-4" style="float:right">
                  <b-form-checkbox
                    id="checkbox-1" v-model="chkCOMPANY_YN" name="checkbox-1" value="Y" unchecked-value="">본사여부
                  </b-form-checkbox>              
              </div>
            </div>                          
          </div>
...
export default {
  ...
  created() {
    ...
    this.gridProps = {
      ...
      columns: [
        { header: "성명", name: "USER_NAME", validation:{ required: true }, align: "left", width: "225", ellipsis: true, editor:{ type:TextBoxEditor, options:{ maxLength:10 } } },
        ...
        { 
          ...
          editor: 'password' 
                ...
                { 
          header: "본사여부", name: "COMPANY_YN", align: "center", width: "100",  editor: { 
          type: 'radio',
        ...
        { 
          header: "사용여부", name: "USE_YN", align: "center", width: "100",  editor: { 
          type: 'radio',
          formatter: 'listItemText',
          options: { 
            listItems: [
        ...
        { 
          header: "ADMIN 여부", name: "ADMIN_YN", align: "center", width: "100", editor:{ 
          type: 'radio', 
          formatter: 'listItemText',
          options: { 
            listItems: [
  ...
  methods: {
    ...
    setgridcolumn: function() {
      ...
      Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, [ "USER_NAME", "USER_ID", "USER_PASS", "USE_YN", "ADMIN_YN" ]);
      // }
    },
    ...
    async btn_search() {
      try {        
        // 파라미터로 성명 사용
        let user_Search = await User_Search(this.user_name.toString(), this.chkCOMPANY_YN.toString())      
    ...
    async btn_add() {
      try 
      {
        ...
        this.$refs.tuiGrid.invoke("appendRow",{_attributes:{checked:true}, USER_NAME:"", USER_ID: "", USER_PASS: "", USE_YN: "Y", ADMIN_YN: "N", ID_KEY: "" },{ at:0, focus:true, extendPrevRowSpan: true });
      }
      catch (err)
      {
        this.$bvModal.msgBoxOk(err, GlobalValue.Err_option);
          ...
          if(msgBox) // 사용자 저장 알림창 확인버튼
          {
            ...
            for(var i=0; i < arr.length; i++)
            {
              data[i]= {
                data: JSON.stringify(
                  {
                    USER_NM:arr[i].USER_NAME.toString(), 
                    ...
                    COMPANY_YN:arr[i].COMPANY_YN.toString(),
                  }
                )
              }
            }            
      ...
      try
      {
        ...
        else
        {
          ...
          if(msgBox) // 사용자 삭제 알림창 확인버튼
          {
            ...
            for(var i=0; i<arr.length; i++)
            {
              data[i]= { data: JSON.stringify({ USER_ID:arr[i].ID_KEY.toString() }) }
            }
