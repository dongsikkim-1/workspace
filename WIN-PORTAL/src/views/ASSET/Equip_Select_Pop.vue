...
import {SEARCH_EQUIP_MOVE} from  "@/api/Equip_Move.js";      // 장비조회
import {Select_Equip_Count} from "./Equip_InOut_Mnt.vue"     // 체크박스 선택값 넘기기
import {eventBus} from "./Asset_Out.vue";

// Vue 핸들링 영역
export default {
  // vue props 선언부
  props:["MOVE_NO", "LOC_CD", "TARGET","SELECTED_LIST"],
  ...
  this.gridProps = {
       ...
       columns: [ { header: "장비코드", name: "EQUIP_NO",   align: "center", width: 100, ellipsis:true},
                  { header: "장비명",   name: "EQUIP_NM",   align: "left",   width: 150,ellipsis:true},
                  { header: "구분",     name: "EQUIP_TYPE", align: "center", width: 70, ellipsis:true}, 
                  { header: "현재위치", name: "EQUIP_LOC",  align: "left",   width: 100 },
                  { header: "사양",     name: "SPEC",       align: "left",   width: 220, ellipsis:true}, 
                  { header: "고장여부",  name: "BROKEN_YN", align: "center", width: 70, ellipsis:true}, 
                  { header: "비고",     name: "REMARK",     align: "left",   width: 180, ellipsis:true},         
                ],
    myTheme: {
      name: "mygrid",
  ...
  methods:{
    ...
    async search(){        
      try {     
        console.log(this.LOC_CD);           
        let grdData = await SEARCH_EQUIP_MOVE(this.MOVE_NO, this.LOC_CD);
      
        // 조회된 데이터가 null이거나 undefined 인 경우
        if(Utility.fn_IsNull(grdData[0].EQUIP_NO)){
          return;
        }
        else{
      ...
      try{           
        ...
        if(arr.length==0) {         
          ...
          try{
            ...
            //인자값 : 선택한 장비
            Select_Equip_Count(arr, this.TARGET);
            // 팝업창을 닫는다.
            this.close();
          }
          // 에러 발생시
    ...
    setgridcolumn: function() {
      ...
      if (this.Device_Type == '"Mobile_V"' || this.Device_Type == "Mobile_V") {
        ...
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NM", "EQUIP_TYPE","SPEC","BROKEN_YN"]);   
        // 그리드 숨겨지는 컬럼(등록번호, 장비코드, 현재위치, 비고) 
        Utility.fn_Grid_HideColumn(this.$refs.tuiGrid, [ "EQUIP_NO", "EQUIP_LOC", "REMARK" ]);        
      }
      // 사용자 디바이스가 Mobile (가로모드)인 경우
      else if (this.Device_Type == '"Mobile_H"' || this.Device_Type == "Mobile_H") {
        // 그리드 보여지는 컬럼(장비명, 구분, 현재위치, 사양) 
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NM", "EQUIP_TYPE","EQUIP_LOC","SPEC", "BROKEN_YN"]);   
        // 그리드 숨겨지는 컬럼(등록번호, 장비코드,  비고) 
        Utility.fn_Grid_HideColumn(this.$refs.tuiGrid, ["EQUIP_NO", "REMARK" ]);
      }
      // 사용자 디바이스가 Tablet인 경우
      else if (this.Device_Type == '"Tablet"' || this.Device_Type == "Tablet"){
        // 그리드 보여지는 컬럼(장비코드, 장비명, 구분, 현재위치, 사양) 
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NO","EQUIP_NM", "EQUIP_TYPE","EQUIP_LOC","SPEC","BROKEN_YN"]);   
        // 그리드 숨겨지는 컬럼(등록번호,  비고) 
      ...
      // 그 외 Dsketop 모니터 또는 Dsketop 모니터보다 큰 경우
         ...
         Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NO", "EQUIP_NM", "EQUIP_TYPE","EQUIP_LOC","BROKEN_YN", "SPEC", "REMARK"]);   
