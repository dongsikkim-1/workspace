        ...
        <fieldset>
          <div class="row">
            <div class="col-md-3 col-sm-6">
              <label class="col-md-4 col-sm-4 col-xs-4 control-label">장비구분:</label>
              <div class="col-md-8 col-sm-8 col-xs-8">
                 <b-form-select v-model="Search_Type" :options="cboTYPE" class="form-control" ></b-form-select>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <label class="col-md-4 col-sm-4 col-xs-4 control-label">현위치:</label>
              <div class="col-md-8 col-sm-8 col-xs-8">
                 <b-form-select v-model="Search_Loc" :options="cboLOCATION" class="form-control" ></b-form-select>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <label class="col-md-4 col-sm-4 col-xs-4 control-label">사양</label>
              <div class="col-md-8 col-sm-8 col-xs-8">
                <b-form-input v-model="Search_Spec" @keyup.enter="search"></b-form-input>  <!-- 사용자 입력창에 enter를 칠 시 조회 -->
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      cboTYPE : [{}], // 조회조건 장비구분 콤보 
      cboLOCATION: [{}], // 조회조건 현위치 콤보 바인딩 배열 변수

      Search_Type: "",  // 장비구분
      Search_Spec: "",  // 조회조건 사양
      Search_Loc: "",  // 조회조건 현위치 콤보 초기값
  ...
  created() {
    ...
    // 그리드 Option Setting
     this.gridProps = {
      ...
      columns: [
        { header: "장비코드", name: "EQUIP_NO", align: "center", width: "50", ellipsis:true},
        { header: "장비타입", name: "EQUIP_TYPE", align: "center", width: "50", ellipsis:true},
        { header: "장비명", name: "EQUIP_NM", align: "left", width: "120"},
        { header: "현위치", name: "EQUIP_LOC", align: "center", width: "50"},
        { header: "사양", name: "SPEC", align: "center", width: "150" },
        { header: "비고", name: "REMARK", align: "center", width: "150" }
  ...
  methods: {
    ...
    grid_Click(ev){ 
      ...
      if(ev.targetType !== "rowHeader" && ev.columnName !== "_checked" && ev.targetType !== "etc" && ev.targetType !== "columnHeader")
      {
        this.$router.push({                                                                         // 키 값인 장비 코드 값을 들고 이동한다
            name: '장비(본체) 등록 추가2',
            params:{EQUIP_NO : this.$refs.tuiGrid.invoke("getValue",ev.rowKey,"EQUIP_NO")}
          });
      }
    ...
    add() {
      ...
      // 체크된 행이 없는 경우(신규)
        if(arr.length === 0)
        {
          ...
          this.$router.push('/Enrollment_Asset_Add2')    // 추가 버튼을 누를시 라우터가 가리키는 곳으로 페이지 전환
        }
    ...
    setgridcolumn: function() {
      ...
      if (this.Device_Type == '"Mobile_V"' || this.Device_Type == "Mobile_V") 
      {
        ...
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_TYPE", "EQUIP_NM", "EQUIP_LOC"]);   
        
        // 그리드 숨겨지는 컬럼(CPU, RAM, 저장장치, 용량) 
        Utility.fn_Grid_HideColumn(this.$refs.tuiGrid, ["SPEC","REMARK"]);
      }
      // 사용자 디바이스가 Mobile (가로모드)인 경우
      else if (this.Device_Type == '"Mobile_H"' || this.Device_Type == "Mobile_H") 
      {
        // 그리드 보여지는 컬럼(장비명, 사용자, 현위치, CPU, RAM) 
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_TYPE", "EQUIP_NM", "EQUIP_LOC", "SPEC"]);
        
        // 그리드 숨겨지는 컬럼(저장장치, 용량) 
        Utility.fn_Grid_HideColumn(this.$refs.tuiGrid, ["REMARK"]);
      }
      // 사용자 디바이스가 Tablet인 경우
      else if (this.Device_Type == '"Tablet"' || this.Device_Type == "Tablet") 
      {
        // 그리드 보여지는 컬럼(장비명, 사용자, 현위치, CPU, RAM, 저장장치, 용량) 
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_TYPE", "EQUIP_NM", "EQUIP_LOC", "SPEC", "REMARK"]);
      }
    ...
    async set_LocCombo(){
      try
      {
        ...
        this.Search_Loc = "%";         // 조회조건 현위치 초기값 '전체' 설정
      }
      catch(err)
      {
    ...
    async set_TypeCombo(){
      try
      {
        let asset_location_combo = await getCommonCode("A02");
        // 조회된 데이터가 null이거나 undefined 인 경우
        if(Utility.fn_IsNull(asset_location_combo[0].CODE_NO))
        {
          return;
        ...
        this.cboTYPE = arr;       // 조회조건 현위치 콤보 바인딩
        this.Search_Type = "%";         // 조회조건 현위치 초기값 '전체' 설정
      }
      catch(err)
      {
         this.$bvModal.msgBoxOk(err, GlobalValue.Err_option);
    ...
    async search() {
      try
      {
        ...
        let desktop_status = await Search_Equip(this.Search_Type, this.Search_Loc, this.Search_Spec)
