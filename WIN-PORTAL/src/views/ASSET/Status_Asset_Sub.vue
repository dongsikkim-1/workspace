...
export default {
  ...
  data(){
    ...
    this.gridProps = {
      ...
      columns: [
        ...
        { header: "사용자", name: "REMARK", align: "center", width: 150, ellipsis:true},
        { header: "현위치", name: "EQUIP_LOC", align: "center", width: 100 },
        { header: "장비구분", name: "EQUIP_TYPE", align: "left", width: 140, ellipsis:true}, 
        { header: "사양", name: "SPEC", align: "left", width: 400, ellipsis:true}, 
      ],
      myTheme: {
        name: "mygrid",
  ...
  methods:{
    ...
    async search(){        
      ...
      {                  
        debugger
        let grdData = await ASSET_STATUS_SUB(this.LOC_CODE,this.EQUIP_TYPE);
    ...
    setgridcolumn: function() {
      ...
      if (this.Device_Type == '"Mobile_V"' || this.Device_Type == "Mobile_V") 
      {
        ...
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NM","EQUIP_LOC"]);   
        // 그리드 숨겨지는 컬럼(위치, 저자, 출판년도) 
        Utility.fn_Grid_HideColumn(this.$refs.tuiGrid, ["SPEC", ]);
      ...
      {
        // 그리드 보여지는 컬럼(위치,데스크탑,노트북,모니터,합계) 
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NM","EQUIP_TYPE"]);   
        // 그리드 숨겨지는 컬럼(위치, 저자, 출판년도) 
        Utility.fn_Grid_HideColumn(this.$refs.tuiGrid, ["SPEC", ]);
      }
      // 사용자 디바이스가 Tablet인 경우
      ...
      {
          // 그리드 보여지는 컬럼(위치,데스크탑,노트북,모니터,합계) 
        Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NM"]);   
        // 그리드 숨겨지는 컬럼(위치, 저자, 출판년도) 
        Utility.fn_Grid_HideColumn(this.$refs.tuiGrid, ["SPEC", "EQUIP_TYPE"]);
      }
      // 그 외 Dsketop 모니터 또는 Dsketop 모니터보다 큰 경우
      else 
      {
         // 그리드 보여지는 컬럼(위치,데스크탑,노트북,모니터,합계) 
         Utility.fn_Grid_ShowColumn(this.$refs.tuiGrid, ["EQUIP_NM", "EQUIP_LOC","EQUIP_TYPE","SPEC"]);   
